...
class TreeAugmentedNaiveBayesSearch(StructureEstimator):
    ...
    def weights(self):
        ...
        graph = nx.Graph()
        df_features = self.data.loc[:, self.data.columns != self.class_node]
        df_self = self.data.loc[:, self.data.columns == self.class_node]
        total_cols = len(df_features.columns)
        total_self = len(df_self.columns)
        w = []
        for i in range(total_cols):
            from_node = df_features.columns[i]
        ...
        for i in range(total_self):
            from_self = df_self.columns[i]
            graph.add_node(from_self)
            for j in range(i + 1, total_cols):
                to_n = df_features.columns[j]
                graph.add_node(to_n)
                mi_self = mutual_info_score(
                    df_features.iloc[:, i], df_features.iloc[:, j])
                graph.add_edge(from_self, to_n, weight=mi_self)
                s = (from_self, to_n)
                b = (s, mi_self)
